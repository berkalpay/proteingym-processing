...
target_seq_path = f"data/temp/{dms_id}.fa"
with open(target_seq_path, "w") as f:
    f.write(f">{dms_id}\n")
    f.write(target_seq)
job_name_prefix = f"data/EVcouplings/{dms_id}/{bitscore}"
os.system(
    f"evcouplings --protein {dms_id} -b {bitscore} -s {target_seq_path} --theta {theta} --colcov 70 --prefix {job_name_prefix} configs/align.txt --yolo"
